...
export function extractParentDirs(filePath: string) {
...
export function searchTasks(tasks: CustomTask[], name: string, group?: string) {
  const searchGroup = group === '.' ? null : group;
  return tasks
    .filter((task) => task.name === name)
    .map((task) => {
      const group = extractParentDirs(task.path!).join('/');
      return {
        ...task,
...
export async function loadCustomTasksFromConfig(context: AppContext) {
  ...
  return await loadCustomTasks(pathTasksDir);
}

async function loadCustomTasks(tasksDir: string) {
  const jsFiles = await getAllJsFiles(tasksDir);
...
async function getAllJsFiles(dir: string) {
